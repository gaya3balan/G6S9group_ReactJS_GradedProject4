...
import {fetchMoviesFromServer, addMovieToFavouritesInServer, removeMovieFromFavouritesInServer, fetchFavouriteMovies} from '../services/movies'
...
const Movies = (searchValue: any ) => {
    ...
    const fetchMovies = async () => {
        try{
            console.log("1")
            const movies = await fetchMoviesFromServer(tabName, searchText);
            console.log(movies)
    ...
    const ddMovieToFavouritesInServer = async (movie: any) =>{
    ...
    const removeMovieFromFavourites = async (movie: any) =>{
        let movieDetails = await fetchFavouriteMovies(movie.title);
                        ...
                        {(() => {
                            if (tabName === "favourit") {
                                return (
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="blue" className="bi bi-x-square-fill" viewBox="0 0 16 16">
                                ...
                                return (
                                    <><button className="Favourites" onClick={() => ddMovieToFavouritesInServer(movie)}>Add to Favourites</button>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="11" fill="violet" className="bi bi-heart-fill" viewBox="0 0 16 16">
